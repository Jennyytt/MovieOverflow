<script>
	import ReviewDraftCard from '../../lib/customComponents/reviewdraft/ReviewDraftCard.svelte';
	...
	async function fetchReview() {
		...
		try {
			...
			const reviewRecords = await pb.collection('critics_reviews').getFullList(undefined, {
				filter: `userId = "${userId}" && isDraft = true`,
				signal: controller.signal, // Pass the AbortController signal
				requestKey: 'getReviewsDraft' // Optional: add a request key for better debugging
			});
