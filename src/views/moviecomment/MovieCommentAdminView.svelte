<script>
	...
	// State variable to track deleted comment IDs (temporary until page reload)
	let deletedComments = [];

	// Function to handle comment deletion
	function handleDelete(commentId) {
		deletedComments = [...deletedComments, commentId];
	}
...
<div class="inline-flex h-full w-full gap-7">
	...
	<div class="inline-flex w-[723.9px] flex-col items-center justify-start gap-[15px]">
		...
		<!-- Dynamically render comments based on displayCount, excluding deleted comments -->
		{#each comments
			.filter((comment) => !deletedComments.includes(comment.id))
			.slice(0, displayCount) as comment (comment.id)}
		...
		{#if displayCount < comments.filter((comment) => !deletedComments.includes(comment.id)).length}
			<button
				type="button"
				on:click={loadMore}
